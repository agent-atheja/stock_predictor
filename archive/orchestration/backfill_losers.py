...
Run:  python -m archive.orchestration.backfill_losers   (RETIRED — see archive/README.md)
"""
...
def main() -> int:
    with stage(log, "backfill-losers"):
        ...
        from archive.ingest.kite_client import verify
        if not verify():
        ...
        from archive.ingest.kite_client import _instruments_nse
        try:
            _instruments_nse()
        ...
        from archive.ingest.kite_backfill import backfill
        try:
        ...
        from archive.ingest.bronze_to_silver import build_silver
        build_silver()
        build_gold()
